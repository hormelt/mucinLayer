function [newMSDs, MSDs, Tracks] = gaussTracker(FileStub,varargin)
...
for Frame = 1:size(bpData,3)
    ...
    temp = callGaussTH(bpData(:,:,Frame),Peaks,p.FeatSize+8,0);
    Centers = [Centers; [temp repmat(Frame,[size(temp,1) 1])]];
end

Tracks = trackin_TH([Centers(:,1), Centers(:,2), Centers(:,5)],p.MaxDisp,param);
...
CollectiveMotionFlag = 0;
MSDs = calcMSD(Tracks,p.NmPerPixel,CollectiveMotionFlag);
% disp([char(10) 'Error correction of MSDs ... '])
% 
% MSDs = MSDs(:,3:end)-2*repmat(rmsgood',size(MSDs,1),1).^2*p.NmPerPixel^2;
% 
% else
%     MSDs = NaN; MSDs = NaN; trialCenters= NaN;
%     
% end
...
% MSDs = MSDs(:,3:end)-2*repmat(rmserror',size(MSDs,1),1).^2*p.NmPerPixel^2;
averagecorrectedMSDs = nansum(MSDs,2)./size(MSDs,2);
stdcorrectedMSDs = std(MSDs,[],2);
newMSDs = [averagecorrectedMSDs stdcorrectedMSDs MSDs];
newMSDs(1,:) = zeros(1,size(newMSDs,2));
%% output
...
switch p.PlotOpt
    case {'simple','bandpass'}
        fig3 = figure();
        whitebg(fig3,[1,1,1])
        loglog(0:size(MSDs,1)-1,MSDs(:,1),'.','color','k')
        hold on
        ylim([min(MSDs(2:end,1))*0.1,max(MSDs(:,1))*10])
        title('Averaged MSD');
        xlabel('\tau (s)');
