function res = bpass3D_TA(image_array,lnoise,lobject)
%
% ; NAME:
% ;               bpass
% ; PURPOSE:
...
normalize = @(x) x/sum(x(:))/size(x,1)/size(x,2)/size(x,3);

eoX = (round(size(image_array,2)/2)-size(image_array,2)/2);
if eoX ==0
    image_array(:,end,:)=[];
end

eoY = (round(size(image_array,1)/2)-size(image_array,1)/2);
if eoY ==0
    image_array(end,:,:)=[];
end

eoZ = (round(size(image_array,3)/2)-size(image_array,3)/2);
if eoZ ==0
    image_array(:,:,end)=[];
end

image_array = double(image_array);
y1 = -round(lobject):round(lobject);
y2 = y1;
y3 = y1;
[y1 y2 y3] = meshgrid(y1,y2,y3);
r = sqrt(y1.^2 + y2.^2 + y3.^2);
gaussian_kernel = normalize(exp(-(r/(2*lnoise)).^2));

boxcar_kernel = (ones([size(r)]));
boxcar_kernel(r(:)>lobject)=0;
...
GKP = padarray(gaussian_kernel,[(size(image_array) - size(gaussian_kernel))/2]);
gconv =  fftshift(ifftn(fftn(image_array).*fftn(GKP)));


BKP = padarray(boxcar_kernel,[(size(image_array) - size(gaussian_kernel))/2]);
bconv =  fftshift(ifftn(fftn(image_array).*fftn(BKP)));
...
% res = filtered;
res = max(filtered*size(r,1)*size(r,2)*size(r,3),0);
